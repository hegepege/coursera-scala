...
abstract class TweetSet {
  ...
  //def listAcc(acc: TweetSet): TweetList
  /**
...
*/
  ...
  def descendingByRetweet: TweetList = {
    val mt = mostRetweeted
    new Cons(mt, remove(mt).descendingByRetweet);
  }
 /* def mostRetweeted: Tweet = {
    val higher = filter(t => t.retweets > elem.retweets)
    if(higher.isEmpty()) elem
    else higher.mostRetweeted
  }

  def listAcc(s: TweetSet): TweetList = {
    if(s.isEmpty()) Nil
    else new Cons(s.mostRetweeted, listAcc(s.remove(s.mostRetweeted)))
  }*/
  /**
    * The following methods are already implemented
    */
